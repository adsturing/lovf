...
def train_sess(sess, model_spec, params, epoch):
  ...
  preds_dict = {}
  step_avg_loss = 0.0
  step_avg_ad_loss = 0.0
  while True:
    step += 1
    try:
      _, loss,ad_loss, preds, labels, group_key, learning_rate = sess.run([
          train_op, loss_op,ad_loss_op,preds_op, labels_op, group_key_op, learning_rate_op
      ])

      step_avg_loss += loss
      step_avg_ad_loss += ad_loss
      if step % params.show_log_steps == 0:
        step_avg_loss /= params.show_log_steps
        step_avg_ad_loss /= params.show_log_steps
        logging.info('step-{}: loss_all {:.4f}'.format(step, step_avg_loss))
        logging.info('step-{}: loss_ad {:.4f}'.format(step, step_avg_ad_loss))
        ...
        for i in range(len(group_key)):
          ...
          preds_dict[group_key[i]].append((labels[i], preds[i], ad_loss))
